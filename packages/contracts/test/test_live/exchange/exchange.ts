...
const deploy = deployer.deployAsync.bind(deployer);
// Save artifacts to conserve testnet eth
const deployOnce = networkConfig.name === 'Testrpc' ? deploy : deployer.deployUnlessDeployedAsync.bind(deployer);
...
function addUseCaseResult(result: UseCaseResult) {
    useCaseResults.push(result);
}

describe(`${networkConfig.name} Integration Tests`, () => {
    ...
    async function awaitMined(txPromise: Promise<string>) {
